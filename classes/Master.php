...
Class Master extends DBConnection {
	...
	function book_tour(){
		// Assuming a database connection is established ($this->conn)
	
		// Validating and sanitizing inputs (example)
		$sanitizedData = [];
		foreach($_POST as $key => $value) {
			$sanitizedData[$key] = mysqli_real_escape_string($this->conn, $value);
		}
	
		// Constructing the query with user ID
		...
		// Executing the query using prepared statement (example)
		$query = "INSERT INTO `book_list` SET $data";
		...
		if($stmt) {
			...
			// Log or handle the specific error ($this->conn->error) for internal use
		}
